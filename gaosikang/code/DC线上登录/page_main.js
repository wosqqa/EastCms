...
define(function(require){
	...
	Main.prototype = {
		...
		init: function(){
			...
			// 判断是否为登录状态
			if(cookie.get('HAS_LOGIN')){
				$username.html(cookie.get('REAL_NAME'));
		...
		loginOutUser: function(){
			var scope = this;
			$.ajax({
				url: Global.Const.BASE_URL + 'qqoauth/loginout',				
				dataType: 'jsonp',
				jsonp: "callbackparam",
				success: function(data){					
					if(data.code === '300'){ // 退出登录失败
						...
						$username.html('未登录！');
						window.location.href = 'QQLoginOut.html';	
						cookie.remove('HAS_LOGIN'); // 清除本地用户信息
						cookie.remove('USER_NAME');
						cookie.remove('REAL_NAME');
					}
				}
			});
		}
