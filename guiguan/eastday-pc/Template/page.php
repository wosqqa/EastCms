...
class page
{
	...
	protected function _get_link($page,$text)
	{
		switch ($this->method) {
			...
			case 'html':
				if($page == 1){
					if(substr_count($this->parameter,'-') > 1){
						$url = str_replace('-?', '',$this->parameter);
					}else{
						$urls = explode('-',$this->parameter);
						$url = $urls[0].'.html';
					}
				}else{
	 ...
	 * @return string
	 */
	public function down_page($name = '下一页',$uk=null)
	{
		if($this->now_page < $this->total_pages)
		{
			return $this->_get_link($this->now_page + 1, $name);
		}
		return sprintf("<script type='text/javascript'>redirect_by_qid('%s','%s');</script>",$name,$uk);
	}

	public function down_page_for_listpage($name = '下一页',$uk=null)
	{
		if($this->now_page < $this->total_pages)
		{
			return $this->_get_link($this->now_page + 1, $name);
	 ...
	 * @return string
	 */
	public function show($param = 1,$uk=null)
	{
		...
		{
			if(!$uk) return $this->$className();
			else return $this->$className($uk);
		}
		return '';
	}

	protected function show_2($uk)
	{
		$return = '';
		$return .= $this->up_page('上一页');
		for ($i = 1; $i <= $this->total_pages; $i++) {
			if ($i == $this->now_page) {
				$return .= "<a class='cur'>$i</a>\n";
			} else {
		...
		if(strpos($uk,'n') === false){
			$return .= sprintf("<script type='text/javascript'>redirect_by_qid('%s','%s');</script>",$i,$uk);
		}
		$return .= $this->down_page('下一页',$uk);
		return $return;
	}
	...
	protected function show_4()
	{
		...
		if($this->up_page()){
			$return .= '<div class="up_page">';
			$return .= $this->up_page();
			$return .= '</div>';
		}
		$return .= "<div class=\"middle_page\">";
		$return .= "<a href=\"\">第".$this->now_page."页</a>";
		$return .= "<div class=\"middle_page_sub\">";
		for ($i = $begin; $i <= $begin + $plus * 2;$i++)
		{
			if($i>$this->total_pages)
			{
			...
			if($i == $this->now_page)
			{
				...
				$return .= "<a class='now_page'>第".$i."页</a>\n";
			}
			else
			{
				$return .= $this->_get_link($i, '第'.$i.'页') . "\n";
			}
		}
		$return .= "</div>";
		$return .= "</div>";
		if($this->down_page_for_listpage()){
			$return .= '<div class="down_page">';
			$return .= $this->down_page();
			$return .= '</div>';
		}
		//$return .= $this->last_page();
