    ...
    $.ajax({
        ...
        success:function(data){
            if (data.position){
                ...
                $.cookie("minieastday_pro_cityname",data.position.cityname,{expires:cookietime,path:'/',domain:'eastday.com'});
                $.cookie("minieastday_pro_provname",data.position.provname,{expires:cookietime,path:'/',domain:'eastday.com'});
                //非北京上海
            }
        }
    });
...
if (!$.cookie('minieastday_pro_id')){
    ...
    var ret = '';
    var cityname = $.cookie("minieastday_pro_cityname") || null;
    var provname = $.cookie("minieastday_pro_provname") || null;
    if(cityname == provname){
        ret = provname;
    } else {
        ret = provname + cityname;
    }
    $.cookie('userNameNew', ret, {expires: 30,path: '/',domain: 'eastday.com'});
}
$(function () {
    special_hover();
    special_360_hover();
    ...
    (function(){
        ...
        // var commend_url ='//123.59.62.164/pcnewsoperate/pcgetreason';//测试地址请求举报原因数据接口
        // var report_url ='//123.59.62.164/pcnewsoperate/pcsubreason';//测试地址上报接口数据
        // var time_url ='//123.59.62.164/pcnewsoperate/reti';//测试地址请求服务器时间
        var commend_url = '//jubao.dftoutiao.com/pcnewsoperate/pcgetreason';//线上地址请求举报原因数据接口
        var report_url = '//jubao.dftoutiao.com/pcnewsoperate/pcsubreason';//线上地址上报接口数据
        var time_url = '//jubao.dftoutiao.com/pcnewsoperate/reti';//线上地址请求服务器时间
        $('body').on('click','#j_user_op', function(){
            //请求举报原因数据接口
            $('body').append(
        ...
        $('body').on('click','.error_button_a',function(){
            ...
            error_detail = error_detail.replace(/\n/gm,'').replace(/\t/gm,'').replace(/\r/gm,'').replace(/ /gm,'');
            if(error_detail.length >= 200){
            ...
            //console.log(error_reason);
            if(!$.cookie('minieastday_pro_cityname')){
                get_area();
            }
            var minieastday_pro_cityname = $.cookie('minieastday_pro_cityname') || null;//地理位置
            $.ajax({
                type: 'get',
                url: time_url,
                timeout:30000, 
            ...
            function report_func(time){
                var param = global_wayPath + "\t" + global_uid + "\t" + softtype + "\t" + softname + "\t" + newstype + "\t" + global_from + "\t" + global_share_url + "\t" + pgnum + "\t" + pgnewsidx + "\t" + newsidx + "\t" + error_reason + "\t" + error_detail + "\t" + global_ishot + "\t" + global_recommendtype + "\t" + newstype + "\t" + global_ver + "\t" + global_os + "\t" + global_browser + "\t" + minieastday_pro_cityname + "\t" + error_phpone;
                var e = $.md5(param + time);
                $.ajax({
                    type: 'get',
                    url: report_url + '?param=' + encodeURI(param) + '&e=' + e,
                    timeout:1000, 
                    dataType: 'jsonp',
                    jsonp: 'jsonpcallback',
...
//浙江:15  上海:3  北京:1  广东:26
/*var jump = function(){
    var minieastday_pro_id = $.cookie('minieastday_pro_id');

    if( $.cookie("jump_double12")
        || minieastday_pro_id == 15
        || ((coo_name == "qid=2345kz" || coo_name == "2345" || coo_name == "qid=2345" || coo_name == "qid=2345xiaohua" || coo_name == "qid=2345shouye" || coo_name == "qid=2345jsllq") && (minieastday_pro_id == 3))
        || (coo_name == 'qid=360sousuo' && minieastday_pro_id == 1)
        || ((coo_name == "qid=jsdbmini" || coo_name == "qid=jsdbnews" || coo_name == "qid=dubadh") && (minieastday_pro_id == 1 || minieastday_pro_id == 26))) {
        ...
        //双12新跳转规则
            if($("a[class='cur']").text() == 1){
                $("a").click(function (){
                    ...
                    var arr = this_href.split("-");
                    if(arr.length >=2 && arr[1] == '2.html'){
                        $.cookie("jump_double12", 1, {expires:1});
                        var uk = arr[0];
                        $(this).attr("href","https://s.click.taobao.com/DacEVKx");
                        window.open(this_href);
                    }
                });
            }
