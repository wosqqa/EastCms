...
class curl_up {
     ...
     * @return mixed|string
     */
    private  static function re_tmp_name(){
        $fileinfo = self::fileinfo();
        $ccg_name = str_replace(array('{','}','-'),'',ccguid());
    ...
    private static function c_post($url,$post_data){
        ...
        if($result) {
            @unlink(str_replace('@','',$post_data['img']));
            return $result;
        }
        return $error;
    }
    ...
    public static function batch_files_path($url){
        ...
        foreach($data as $_k=>$_v){
            $nt_path = str_replace(basename($_v['tmp_name']),str_replace(array('{','}','-'),'',ccguid()).".{$_v['extension']}",$_v['tmp_name']);
