                    ...
                    if (d.success != 1){
                        $(_this).popupMeans({
                            ...
                            callback: function() {
                                ...
                                $("#popupMessage").find('.popup-main').html('<div style="text-align: center;margin-top:10px;">'+d.msg+'</div>');
                                //$('.phone_1').show();
                        ...
                        $(_this).popupMeans({
                            ...
                            callback: function() {
                                var $messageMain = $("#popupMessage").find("[data-main]"),
                                    _height = 172-81;
                                $messageMain.height(_height);
                                var tp = data.email.substr(0,4)+'****'+data.email.substr(data.email.length-7);
                                var b = data.email.split('@');
                                c = b[1].split('.');
                                $("#popupMessage").find('.popup-main').html('<div style="text-align: center;margin-top: 10px;margin-left: 50px;margin-right: 80px;line-height: 20px;">已向邮箱<b>'+tp+'</b>发送了一封激活邮件，点击<a href="https://mail.'+c[0]+'.com/" target="_blank">进入邮箱</a>完成邮箱验证</div>');
                                $("#popupMessage").find('.submit-btn').click(function(){
                                    if(d.success==1){
                                        $(this).attr({href:USER_URL+'/edit/index/type/1'});
                                    }else{
