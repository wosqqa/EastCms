...
$(function() {
    ...
    // 匹配密码，长度在{0}-{1}之间，只能包含字符、数字和下划线。      
    jQuery.validator.addMethod("rangePwd", function(value, element, params) {
        var regex = new RegExp('^\\w{' + params[0] + ',' + params[1] + '}$');
        return this.optional(element) || regex.test(value);
    }, "长度在{0}-{1}之间，只能包含字符、数字和下划线。");

    // 匹配密码，以字母开头，长度在6-12之间，只能包含字母、数字和下划线。 
    jQuery.validator.addMethod("isPwd", function(value, element) {
        return this.optional(element) || /^[a-zA-Z]{1}\w{5,11}$/.test(value);
    }, "以字母开头，长度在6-12之间，只能包含字符、数字和下划线。");

    // 匹配数字长度      
    jQuery.validator.addMethod("numberLength", function(value, element, params) {
        var regex = new RegExp('^\\d{' + params + '}$');
        return this.optional(element) || regex.test(value);
    }, "请输入{0}位数字。");
