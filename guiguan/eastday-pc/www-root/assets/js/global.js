...
(function() {
    ...
    if (!$.cookie('mylist')) {
        ...
        global_uid = decodeURIComponent($.parseJSON($.cookie('mylist')).uid);
    }
    var temp = window.location.search; //获取url中"?"符后的字串
    if(String(GetRequest(temp).rcgid) != 'undefined'){
        global_uid = decodeURIComponent(GetRequest(temp).rcgid);
        var info = '{"uid":"' + global_uid + '","softtype":"' + soft_type + '","softname":"' + soft_name + '"}';
        $.cookie('mylist', info, {expires: 3000, path: '/',domain:'eastday.com'});
        $.cookie('user_id', global_uid, {expires: 3000, path: '/',domain:'eastday.com'});
    }
    ...
    function getOnline_ajax(url) {
        ...
        $.ajax({
            ...
            url: url,
            data: {
                param: encodeURIComponent(paramOnline)
            },
            dataType: 'jsonp',
            jsonp: 'jsonpcallback'
        });
